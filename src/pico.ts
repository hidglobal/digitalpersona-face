/*
This is a port of the Pico.js library to TypeScript.

The original Pico.js library is a JavaScript implementation of a face detection
algorithm based on the Viola-Jones method. It is designed to be lightweight and efficient,
making it suitable for use in web applications and other environments where performance is critical.

The original library is released under the MIT license, see https://github.com/tehnokv/picojs
*/
...
export type Detection = [row: number, column: number, scale: number, score: number];
export type DetectionUpdater = (detections: Detection[]) => Detection[];

export function unpack_cascade(bytes: Uint8Array): RegionClassifier
{
    if (bytes.length < 8) throw new Error("Cascade file is too short");
    ...
    */
    ...
    for (var t = 0; t < ntrees; ++t) {
        ...
        _tcodes.push(...[0, 0, 0, 0]) // reserve space for the root node
        _tcodes.push(...bytes.slice(p, p + 4 * Math.pow(2, tdepth) - 4));
        // Array.prototype.push.apply(_tcodes, [0, 0, 0, 0])
        // Array.prototype.push.apply(_tcodes, bytes.slice(p, p + 4 * Math.pow(2, tdepth) - 4))
        p = p + 4 * Math.pow(2, tdepth) - 4
        // read the prediction in the leaf nodes of the tree
        for (var i = 0; i < Math.pow(2, tdepth); ++i) {
            dview.setUint8(0, bytes[p + 0]), dview.setUint8(1, bytes[p + 1]), dview.setUint8(2, bytes[p + 2]), dview.setUint8(3, bytes[p + 3])
            _tpreds.push(dview.getFloat32(0, true))
            p = p + 4
        }
        // read the threshold
        dview.setUint8(0, bytes[p + 0]), dview.setUint8(1, bytes[p + 1]), dview.setUint8(2, bytes[p + 2]), dview.setUint8(3, bytes[p + 3])
        _thresh.push(dview.getFloat32(0, true))
        p = p + 4
    }
    var tcodes = new Int8Array(_tcodes);
    var tpreds = new Float32Array(_tpreds)
    var thresh = new Float32Array(_thresh)

    // construct the classification function from the read data
    function classify_region(r: number, c: number, s: number, pixels: number[], ldim: number): number {
        r = 256 * r
        c = 256 * c
        var root = 0
        var o = 0.0
        ...
        for (var i = 0; i < ntrees; ++i) {
            ...
            for (var j = 0; j < tdepth; ++j) {
                var offset = root + 4*idx
                var p1 = pixels[((r + tcodes[offset + 0]*s) >> 8)*ldim + ((c + tcodes[offset + 1]*s) >> 8)]
                var p2 = pixels[((r + tcodes[offset + 2]*s) >> 8)*ldim + ((c + tcodes[offset + 3]*s) >> 8)]
                // we use '>> 8' here to perform an integer division: this seems important for performance
                idx = 2 * idx + Number(p1 <= p2)
            }
...
export type RegionClassifier = (row: number, column: number, scale: any, pixels: any, ldim: any) => number

export function run_cascade(image: Image, classify_region: RegionClassifier, params: CascadeParams): Detection[] {
...
export function cluster_detections(dets: Detection[], iouThreshold: number): Detection[] {
    ...
    for (var i = 0; i < dets.length; ++i) {
        ...
        if (assignments[i] == 0) {
            ...
            for (var j = i; j < dets.length; ++j) {
                var dj = dets[j];
                if (calculate_iou(dets[i], dj) > iouThreshold) {
                    assignments[j] = 1
                    r = r + dj[0]
                    c = c + dj[1]
                    s = s + dj[2]
...
export function instantiate_detection_memory(size: number): DetectionUpdater {

    // initialize a circular buffer of `size` elements
    var n = 0, memory: Detection[][] = []
    ...
    */
    return (dets: Detection[]): Detection[] => {
        memory[n] = dets
        n = (n + 1) % memory.length
        dets = []
...
*/
function calculate_iou(det1: Detection, det2: Detection): number {
